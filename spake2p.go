...
func (p point) AsBytes() []byte {
	o1 := elliptic.Marshal(elliptic.P256(), p.x, p.y)
	return o1
}
func (p *point) fromBytes(in []byte) {
...
func (ctx *SpakeCtx) calculateZV() {
	...
	ctx.Z.x, ctx.Z.y = ctx.curve.ScalarMult(znx, zny, ctx.xRandom.Bytes())
...
func (ctx *SpakeCtx) CalculateZVb() {
	...
	ctx.Z.x, ctx.Z.y = ctx.curve.ScalarMult(zznx, zzny, ctx.yRandom.Bytes())
	ctx.L.x, ctx.L.y = ctx.curve.ScalarBaseMult(ctx.W1)
	ctx.V.x, ctx.V.y = ctx.curve.ScalarMult(ctx.L.x, ctx.L.y, ctx.yRandom.Bytes())
}

func (ctx *SpakeCtx) calculateHash(seed []byte) error {

	sh0sum := sha256Enc(seed)
	...
	tt := createTT(sh0sum, "", "", mbin, nbin, ctx.X.AsBytes(), ctx.Y.AsBytes(), ctx.Z.AsBytes(), ctx.V.AsBytes(), ctx.W0)
	...
	key := hkdfSHA256(ctx.Ka, nil, []byte("ConfirmationKeys"), 32)

	ctx.cA = hmacSHA256Enc(ctx.Y.AsBytes(), key[:16])
	ctx.cB = hmacSHA256Enc(ctx.X.AsBytes(), key[16:])

	cryptKey := hkdfSHA256(ctx.Ke, nil, []byte("SessionKeys"), 16*3)
	ctx.decryptKey = cryptKey[16:32]
	ctx.encryptKey = cryptKey[:16]
	return nil
}
